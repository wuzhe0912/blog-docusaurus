...
## Arbeiten mit Arrays, die mehrstufige Arrays und Objekte enthalten
...
const findAgeById = (array, targetId) => {
  for (let item of array) {
    if (Array.isArray(item)) {
      // Wenn item ein Array ist, findAgeById-Funktion rekursiv aufrufen
      const result = findAgeById(item, targetId);
      // Wenn im Sub-Array gefunden, sofort zurückgeben
      if (result !== undefined) {
        return result;
      }
    } else if (item.id === targetId) {
      // Wenn item ein Objekt ist, das entsprechende age zurückgeben
