...
## 1. Повторить строку заданное количество раз

Спроектируйте функцию, которая повторяет строку указанное число раз.

### I. С использованием `repeat()` (ES6+)

Так как String теперь поддерживает `repeat()`, можно использовать его напрямую.
...
### II. С использованием цикла

Если не использовать `repeat()`, можно применить цикл с валидацией параметра, ограничив его положительными целыми числами.
...
## 2. Извлечь имя файла или расширение из строки

Спроектируйте `getFileExtension()`, который извлекает расширение файла из параметра. Если расширения нет, верните имя файла.
...
### I. Использование split для получения расширения
...
## 3. Найти самую длинную строку в массиве

### I. С использованием метода `sort()`
...
### II. С использованием метода `reduce()`
...
Спроектируйте функцию, которая преобразует строку в camelCase.

### I. С использованием метода `replace()`
...
### II. С использованием метода `split()`
...
## 5. Подсчитать количество повторов строк в массиве

### I. С использованием метода `Map()`
...
### III. С использованием `Object.groupBy()` (ES2023+)
...
## 6. Извлечь и дедуплицировать расширения файлов из массива строк

### I. С использованием метода `split()`
