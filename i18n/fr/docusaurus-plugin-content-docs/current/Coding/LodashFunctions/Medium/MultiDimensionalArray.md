...
## Manipuler des tableaux contenant des tableaux et objets multiniveaux

### 1. Solution avec récursion
...
const findAgeById = (array, targetId) => {
  for (let item of array) {
    if (Array.isArray(item)) {
      // Si item est un tableau, appeler récursivement la fonction findAgeById
      const result = findAgeById(item, targetId);
      // Si trouvé dans le sous-tableau, retourner immédiatement
      if (result !== undefined) {
        return result;
      }
    } else if (item.id === targetId) {
      // Si item est un objet, retourner l'age correspondant
...
### 2. Utilisation de la recherche en profondeur (DFS)
...
### 3. Utilisation de la recherche en largeur (BFS)
